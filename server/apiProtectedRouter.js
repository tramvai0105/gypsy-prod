...
import { Order, Payment, Person, Review, User } from './models.js'
import inspector from "schema-inspector";
const apiProtectedRouter = express.Router();
import multer from "multer";
import fs from "fs"
import { resolve } from 'path';
import { overwriteMiddlewareResult } from 'mongoose';
import { checkUser, isAdmin, isLoggined } from './middleware.js';
...
apiProtectedRouter.get('/', (req, res) => {
    res.json(200)
})
...
apiProtectedRouter.get('/paymentsByUser/:id', checkUser, async (req, res) => {
    try {
        let paymentsTo = await Payment.find({ to: req.params.id })
        let paymentsFrom = await Payment.find({ source: req.params.id })
        return res.send(JSON.stringify({to: paymentsTo, from: paymentsFrom}));
    }
    catch (error) {
        console.log(error)
        return res.status(501).json({ message: error });
...
export default apiProtectedRouter